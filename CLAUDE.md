...
5. **Hermes `api_server` 401/disconnected** — reconectar (pré-req p/ usar a API HTTP do Hermes, ex.: ponte de Instagram p/ Hermes).
6. **Verificar:** tile no "Add agent" (após build do Paperclip terminar); DeepSeek sobrevive a restart.
7. **Decisão estratégica (em aberto):** focar no **Hermes** (WhatsApp/Telegram nativos + memória; bom p/ secretário) vs manter OpenClaw. Hermes pode atender Instagram via ponte (como OpenClaw), mas precisa do `api_server` consertado primeiro.
